...
const CONTENT_TRANSITION = { duration: 0.35, ease: [0.2, 0, 0, 1] };
...
export const StatefulButton = React.forwardRef(function StatefulButton(
  {
    className,
    variant,
    size,
            ...
            <span className="inline-flex">
              {typeof resolvedLabel === "string" ? (
                resolvedLabel.split("").map((char, i) => (
                  <motion.span key={i} variants={staggerItem} className="inline-block whitespace-pre">
                    {char}
                  </motion.span>
                ))
              ) : (
