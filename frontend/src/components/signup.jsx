    ...
    <div className="relative h-screen w-screen overflow-hidden flex items-center justify-center font-sans text-white">
      ...
      <div className="relative z-10 w-full max-w-4xl min-h-[500px] bg-white/10 backdrop-blur-lg border border-white/20 rounded-3xl shadow-2xl p-10 flex flex-col md:flex-row gap-8 mx-4">
        ...
        <div className="flex-1 flex flex-col justify-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-2">Mini<br/>Instagram</h1>
          <div className="h-1 w-16 bg-gradient-to-r from-pink-500 to-cyan-400 mb-10 rounded-full"></div>

          <form className="flex flex-col gap-6 w-full max-w-sm" onSubmit={(e)=>{SubmitHandler(e)}}>
            <input type="text" placeholder="Full Name" value={fullName} disabled={loading} onChange={(e)=>{
            ...
            <input type="text" placeholder="Username" value={username} disabled={loading} onChange={(e)=>{
              setusername(e.target.value)
            }} className="w-full bg-transparent border-b border-white/30 pb-2 text-sm focus:outline-none focus:border-white transition-colors placeholder:text-white/50 disabled:opacity-50" />
            <input type="email" placeholder="Email" value={email} disabled={loading} onChange={(e)=>{
              setemail(e.target.value)
            }} className="w-full bg-transparent border-b border-white/30 pb-2 text-sm focus:outline-none focus:border-white transition-colors placeholder:text-white/50 disabled:opacity-50" />
            <input type="password" placeholder="Password" value={password} disabled={loading} onChange={(e)=>{
            ...
            <button type="submit" disabled={loading} className="w-full mt-4 py-3 rounded-xl font-bold text-white bg-gradient-to-r from-pink-500 to-cyan-400 hover:opacity-90 transition-opacity shadow-[0_4px_14px_0_rgba(34,211,238,0.39)] disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2">
              {loading ? (
                <>
